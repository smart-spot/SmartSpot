...
try:
    ...
    print("Failed to load a .*.so implementation. Please either install cascade python support, or set $PYTHONPATH to the folder containing the built .*.so")
    sys.exit(1)
...
if __name__ == '__main__':
    ...
    if args.logfile is not None:
        logging.basicConfig(level=getattr(
            logging, args.logging_level), filename=args.logfile)
    else:
    ...
    while (cap.isOpened):
        ...
        capi.trigger_put('TriggerCascadeNoStoreWithStringKey', key, create_payload(
            frame, i))  # trigger our equivalent of a frontend
